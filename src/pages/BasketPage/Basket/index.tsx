...
const BasketItem = ({ ring, onIncrement, onDecrement, onRemove }: BasketItemProps) => (
  <tr key={ring.ring.id} className="basket__tr">
    ...
    <td align="center" valign="middle" className="basket__td">
      <div className="basket__counter">
        <button onClick={() => onIncrement(ring.ring.id)} className="basket__counter-btn basket__counter-btn--plus" />
        <p>{ring.defaultCount}</p>
        <button onClick={() => onDecrement(ring.ring.id)} className="basket__counter-btn" />
      </div>
    </td>
    <td align="center" valign="middle" className="basket__td">{ring.ring.priceSale} ₽</td>
    <td align="center" valign="middle" className="basket__td">
      <button onClick={() => onRemove(ring.ring.id)} className="basket__delete-btn">
      ...
      <div className="container">
        ...
        <div className="basket__inner">
          ...
          <ul className="basket__list--mobile">
            {rings?.map(ring => (
              <li key={ring.ring.id} className="basket__item--mobile">
            ...
            <tbody>
              {rings?.map(ring => (
                <BasketItem
                  key={ring.ring.id}
                  ring={ring}
                  onIncrement={handleClickIncrementCount}
                  onDecrement={handleClickDecrementCount}
                  onRemove={handleClickRemoveOne}
                />
              ))}
            </tbody>
          </table>
