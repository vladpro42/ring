...
import { changeFavoriteCreator } from "../../rings/ringsReducer"
...
const ProductCart = ({ cart, to }: { cart: ICart, to?: number | string, }) => {
    ...
    const handleClickActiveFavorite = (cart: ICart) => {
        setActive(true)
        dispatch(changeFavoriteCreator(3))


        const localStorageData = localStorage.getItem("favoriteRings");
        if (localStorageData) {
            const favoriteRings: ICart[] = JSON.parse(localStorageData)
    ...
    return <li className="new-items__slide">
        ...
        <div className="new-items__slide-item">
            <svg
                ...
                className={active ? "new-items__slide-favotite new-items__slide-favotite--active" : "new-items__slide-favotite"}
